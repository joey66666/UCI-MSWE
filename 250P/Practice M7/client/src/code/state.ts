...
export function createState(inParentComponent) {
    return {
        ...
        showMessage: async function (inMessage: IMAP.IMessage): Promise<void> {
            console.log("showMessage()", inMessage);
        ...
        showComposeMessage: function (inType: string): void {
            console.log("showComposeMessage()");
            switch (inType) {
                case "new":
                    this.setState({
                        currentView: "compose",
        ...
        addMessageToList: function (inMessage: IMAP.IMessage): void {
            console.log("addMessageToList()", inMessage);
        ...
        setCurrentMailbox: function (inPath: String): void {
            console.log("setCurrentMailbox()", inPath);
        ...
        fieldChangeHandler: function (inEvent: any): void {
            console.log("fieldChangeHandler()", inEvent.target.id, inEvent.target.value);
        ...
        saveContact: async function (): Promise<void> {
            console.log("saveContact()", this.state.contactID, this.state.contactName, this.state.contactEmail);
            const cl = this.state.contacts.slice(0);
            this.state.showHidePleaseWait(true);
        ...
        deleteContact: async function (): Promise<void> {
            console.log("deleteContact()", this.state.contactID);
            this.state.showHidePleaseWait(true);
            const contactsWorker: Contacts.Worker = new Contacts.Worker();
        ...
        updateContact: async function (): Promise<void> {
            console.log("updateContact()", this.state.contactID, this.state.contactName, this.state.contactEmail);
            this.state.showHidePleaseWait(true);
            const contactsWorker: Contacts.Worker = new Contacts.Worker();
            ...
            this.state.showHidePleaseWait(false);
            const updated = this.state.contacts.filter((inElement) => inElement._id == this.state.contactID);
            let updatedID = this.state.contacts.indexOf(updated[0])
            const cl = this.state.contacts;
            cl[updatedID] = {_id: this.state.contactID, name: this.state.contactName, email: this.state.contactEmail};
            this.setState({
                contacts: cl,
        ...
        deleteMessage: async function (): Promise<void> {
            console.log("deleteMessage()", this.state.messageID);
            this.state.showHidePleaseWait(true);
            const imapWorker: IMAP.Worker = new IMAP.Worker();
        ...
        sendMessage: async function (): Promise<void> {
            console.log("sendMessage()", this.state.messageTo, this.state.messageFrom, this.state.messageSubject,
                this.state.messageBody
            );
            this.state.showHidePleaseWait(true);
